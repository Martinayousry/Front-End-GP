...
//             method: "POST",
//             url: "https://serverless.roboflow.com/dog-skin-disease-dataset/2",
//             params: {
...
const SkinDiseasePrediction = () => {
  ...
  const handleImageUpload = (event) => {
    ...
    const files = event.target.files || event.dataTransfer.files;
    if (files.length > 0) {
      // Store both the URL for preview and the file for API call
      setImage(URL.createObjectURL(files[0]));
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        try {
          const apiResponse = await axios({
            method: "POST",
            url: "https://serverless.roboflow.com/dog-disease-detection-1-knn7t/1",
            params: {
              api_key: "Mu2gT32FBzYEGW4VhaS5"
            },
            data: base64data,
  ...
  return (
    <div className="p-25">
      <div className="skin-disease">
        ...
        <p className="text-gray-500">
          Upload a clear photo of your dog's skin condition to get a prediction
          of 7 potential diseases including Dermatitis,Fungal infections,Hypersensitivity,democosis,demodicosis,ringworm.
        </p>
            ...
            <div className="all-predictions">
              ...
              <ul>
                {prediction.predictions.map((item, index) => (
                  <li key={index}>
                    <strong>{item.class}:</strong> {(item.confidence * 100).toFixed(2)}% confidence
                  </li>
                ))}
              </ul>
            </div>

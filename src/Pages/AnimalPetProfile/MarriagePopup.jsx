...
const MarriagePopup = ({ onClose, receiverPetId }) => {
  const { token } = useAuth();
  ...
  const handleSelectPet = async (senderPetId) => {
    try {
      await axios.post(
        "/api/PetMarriageRequest/MakeMarriageRequest",
        {
      ...
      toast.success("Marriage request sent successfully!"); 
      onClose();
    } catch (error) {
      console.error("Failed to send marriage request:", error);

...
const MarriageRequests = ({ petId }) => {
  ...
  useEffect(() => {
    const fetchMarriageRequests = async () => {
      try {
        ...
        if (isAdmin) {
          // Admin sees all Animal marriage requests
          res = await axios.get("/api/AnimalMarriageRequest", {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
        } else {
          // Regular user sees Pet marriage requests
          res = await axios.get("/api/PetMarriageRequest", {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
  ...
  const handleRespond = async (requestId, isAccept) => {
    try {
      const endpoint = isAdmin
        ? isAccept
          ? `/api/AnimalMarriageRequest/AcceptMarriageRequest/${requestId}`
          : `/api/AnimalMarriageRequest/RejectMarriageRequest/${requestId}`
        : isAccept
          ? `/api/PetMarriageRequest/AcceptMarriageRequest/${requestId}`
          : `/api/PetMarriageRequest/RejectMarriageRequest/${requestId}`;
  ...
  // Filter requests only for current pet if user is not admin
        ...
        <div key={request.requestId} className="flex items-center justify-between p-4 mb-3 bg-white rounded-lg shadow">
          <div className="flex items-center">
            <img
              src={
                isAdmin
                  ? request.senderPet?.photoUrls?.[0]
                  : request.senderPet.photoUrls[0]
              }
              alt={request.senderPet?.name || "Pet"}
              className="w-16 h-16 object-cover rounded-full mr-4"
            />
            <div>
              <h3 className="font-bold">{request.senderPet?.name}</h3>
